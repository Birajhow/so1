/*
2. Execução do algoritmo usando threads

1 thread será encarregado pela leitura da cena e pela gravação da imagem em disco.
Os demais threads (trabalhadores) ficarão encarregados pelo processamento da cena e dos raios.
Para isso, a imagem terá que ser dividida igualmente entre os threads, os quais processarão suas partes, escrevendo a imagem nova em uma região de memória compartilhada.
*/
...
#include <pthread.h>
// Fim da adição de bibliotecas

// Adicionei constante para o uso de threads
#define NUM_THREADS 8
// Fim adição de constantes
...
// Struct que contém os dados das threads
struct thread_data{
  int    thread_id;
  int    inicio;
  int    fim;
...
// Vetor que contém os dados das threads
...
int main(int argc, char ** argv)
{
  ...
  // Procedimentos envolvendo os threads na main
  // Inicializações
  ...
  // Definindo que as threads são joinables
  ...
  for(t=0; t<NUM_THREADS; t++){
    ...
    thread_data_array[t].samples     = samples;
    thread_data_array[t].s           = s;
    thread_data_array[t].rcp_samples = rcp_samples;
...
void *doWork(void *threadarg)
{
  ...
  int inicio        = my_data->inicio;
  int fim           = my_data->fim;
  int samples       = my_data->samples; 
  int s             = my_data->s;    
  float rcp_samples = my_data->rcp_samples;
  uchar *image      = my_data->image;
  camera c          = my_data->c;
  ...
  for(inicio ; inicio < fim ; inicio++)
  {
    ...
    for(s=0; s<samples; s++) {
        ray rr    = get_primary_ray(&c,inicio,j,s);    
    ...
    image[ 3* (inicio * c.view.height + j) + 0] = floatToIntColor(r);
    image[ 3* (inicio * c.view.height + j) + 1] = floatToIntColor(g);
    image[ 3* (inicio * c.view.height + j) + 2] = floatToIntColor(b);
    }
  }
