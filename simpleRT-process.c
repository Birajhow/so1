/*
1.Execução do algoritmo usando processos: 

1 processo pai será encarregado pela leitura da cena 3-D e pela gravação da imagem em disco. 
Os demaisprocessos filhos (trabalhadores) ficarão encarregados pelo processamento da cena 3-D e dos raios. 
Para isso, a imagem terá que ser dividida igualmente entre os processos, os quais processarão suas partes, escrevendo a imagem nova em 
...
// Adicionei as bibliotecas de processos
#include <unistd.h>
#include <sys/types.h> 
#include <sys/ipc.h>
#include <sys/shm.h>
...
// Adicionei constante para o uso de processos
#define N_WORKERS 8
...
int main(int argc, char ** argv)
{
  ...
  //Divide o tamanho em fatias de acordo com o n_workers
  blockSizeWidth = c.view.width/n_workers;

  for(v = 0 ; v < n_workers ; v++)
  {
      pid = fork();
      m_pid = v;
      if(pid == -1)
      ...
      {
          /*//ASS HAPPENING (parte que associa os filhos a regiao compartilhada)
          image = shmat(shmid, (void*)0, 0);
          if(image == (uchar*)(-1))
          {
              perror("shmat");
          ...
          for(i = m_pid*blockSizeWidth ; i < blockSizeWidth*(m_pid+1) ; i++)
          { 
              ...
              /*//ASS HAPPENING (Os filhos terminam a associação) NÃO SEI SE É NECESSARIO
                if(shmdt(image) == -1)
                {
                  perror("shmdt");
                  exit(1);
  ...
  if(save_bmp("output_rt_process.bmp",&c,image) != 0)
  {
      fprintf(stderr,"Cannot write image 'output.bmp'.\n");
      return 0;
  }

  /*//ASS HAPPENING (pai se desassocia e destroi a regiao) NAO SEI SE NECESSARIO
  if(shmdt(image) == -1)
    {
        perror("shmdt");
        exit(1);
...
*/
  ...
  //ASS HAPPENING (se o pai desassocia e destroi isso nao é necessario?)
  //---freeing data---
  //free(rays);
  free(image);
